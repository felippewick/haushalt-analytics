 ...
 *
 * Model: Qwen2.5-1.5B-Instruct Q4_K_M (~940 MB).
 */
import {
  createWriteStream,
  existsSync,
...
export const EXPECTED_BYTES = 986_048_768
const MODEL_URL =
  'https://huggingface.co/bartowski/Qwen2.5-1.5B-Instruct-GGUF/resolve/main/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf'

const STALE_MODELS = ['Qwen2.5-0.5B-Instruct-Q4_K_M.gguf']
...
function removeStale() {
  for (const name of STALE_MODELS) {
    const path = join(MODEL_DIR, name)
    if (existsSync(path)) {
      unlinkSync(path)
      console.log(`Removed old model ${name}`)
    }
  }
}
